/* This program replaces all instances of a text inside another string with
 * other text */
...
int _copyText(const char source[], char dest[]);
...
int main(void) {
  ...
  int found, result, text_len1, text_len2, pos;
  ...
  text_len1 = _stringLength(s1); // get text len for function

  printf("Text to insert: ");
  nl_readLine(s2, I_LEN);

  text_len2 = _stringLength(s2);

  // If the text to be replaced is zero, make it a space
  if (text_len2 == 0) {
    s2[0] = ' ';
    s2[1] = '\0';
  }
  ...
  do {
    ...
    result = removeString(source, pos + 1, text_len1);
